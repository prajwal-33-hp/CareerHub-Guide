...
export default function RecruiterMessages() {
  ...
  async function loadConversations() {
    ...
    try {
      ...
      // Handle direct query param auto-selection
      if (targetUserId) {
  ...
  // Select active conversation & load message history
  ...
  function handleLaunchVideoInterview() {
    if (!activeConversation) return
    const other = activeConversation.otherParticipant
    const job = activeConversation.job
    const roomId = `int_${activeConversation._id.slice(-6)}_${Math.random().toString(36).substring(2, 7)}`
    const baseParams = `candidate=${encodeURIComponent(other?.name || 'Candidate')}&recruiter=${encodeURIComponent(
      user?.name || 'Recruiter'
    )}&job=${encodeURIComponent(job?.title || 'Live Interview')}`
    ...
    api
      .post('/messages', {
        ...
        text: `🎥 Live Video Interview Started! Join room here: ${window.location.origin}${studentInterviewUrl}`,
      })
      .then(({ data }) => {
        if (data.message) {
          setMessages((prev) => [...prev, data.message])
  ...
  return (
    <div>
      <Helmet>
        <title>Messages & Applicant Chat | Recruiter Dashboard</title>
      </Helmet>

      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="font-display text-2xl font-bold text-ink">Candidate In-App Messaging</h2>
          <p className="mt-1 text-sm text-ink-soft">
            Live 1-on-1 real-time direct chat with job candidates and applicants.
          </p>
        </div>
        ...
        {activeConversation ? (
          <div className="flex flex-col h-full bg-white">
            ...
            <div className="flex items-center justify-between border-b border-ink/10 p-3.5 px-5 bg-white">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-ink-soft">
                    {activeConversation.job?.title
                      ? `Candidate for ${activeConversation.job.title}`
                      : 'Candidate'}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={handleLaunchVideoInterview}
                  className="btn-secondary text-xs inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-800 border-emerald-300 hover:bg-emerald-100"
                >
                  <Video size={14} className="text-emerald-600" /> Start Video Call
                </button>

                {activeConversation.otherParticipant?.email && (
                  <a
                    href={`mailto:${activeConversation.otherParticipant.email}`}
            ...
            {/* Quick Templates Bar */}
            <div className="flex items-center gap-1.5 overflow-x-auto border-b border-ink/5 bg-paper/50 px-4 py-2 text-[11px]">
              <span className="font-semibold text-ink-soft text-[10px] uppercase tracking-wider shrink-0">
                Quick Reply:
              </span>
              <button
                onClick={() =>
                  insertTemplate(
                    "Hi! We were impressed by your profile and would love to schedule a 30-minute technical interview. What times work best for you this week?"
                  )
                }
                className="shrink-0 rounded-full border border-ink/10 bg-white px-2.5 py-1 text-ink-soft hover:border-signal hover:text-ink transition"
              >
                📅 Schedule Interview
              </button>
              <button
                onClick={() =>
                  insertTemplate(
                    "Hello! Could you please share a link to your GitHub profile or any recent live demo projects you've built?"
                  )
                }
                className="shrink-0 rounded-full border border-ink/10 bg-white px-2.5 py-1 text-ink-soft hover:border-signal hover:text-ink transition"
              >
              ...
              <button
                onClick={() =>
                  insertTemplate(
                    "Thank you for applying! We are currently reviewing all applications and will get back to you with next steps shortly."
                  )
                }
                className="shrink-0 rounded-full border border-ink/10 bg-white px-2.5 py-1 text-ink-soft hover:border-signal hover:text-ink transition"
              >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-paper/20">
              {loadingMessages ? (
                ...
                <div className="flex h-full flex-col items-center justify-center text-center text-xs text-ink-soft">
                  ...
                  <p className="mt-1">
                    Send a message to introduce yourself or schedule an interview with this
                    candidate.
                  </p>
                </div>
              ) : (
          ...
          <div className="flex h-full flex-col items-center justify-center p-8 text-center text-sm text-ink-soft bg-paper/20">
            ...
            <p className="mt-1 text-xs max-w-sm">
              Choose a candidate from the left pane or message them directly from the Applicants
              board.
            </p>
          </div>
        )}
